...
%% Fitting
% Initialize parameters and selection switch
initial_guess = [1, 0.5, 2];  % Initial guess for a2, beta1, beta2, and kappa
fixed = [];
Sel = [1, 1, 1];  % Selection switch (all parameters are free)
set_Size = [2, 4, 6, 4, 4, 6, 6, 6, 6];  % Conditions
...
[estimated_params, fval, exitflag, output] = fminsearch(@(Pvar) ...
    lcmR_S61A1B1K(Pvar, fixed, Sel, data_AQS, set_Size), ...
    initial_guess(Sel == 1), options);

% Extract AIC and BIC from the model using the estimated parameters
[~, aic, bic] = lcmR_S61A1B1K(estimated_params, [], Sel, data_AQS, set_Size);
...
for i = 1:length(set_Size)
    ...
    elseif ismember(i, [4, 6, 8])
        ...
        beta_values(i) = 1-estimated_params(2);
        baseAmp = estimated_params(1) * 2 .^ beta_values(i);
        amplitude_values(i) = baseAmp * (colour_Num(i) .^ -beta_values(i));
    end
end
...
% Store the results in a table
results = table(set_Size', amplitude_values', beta_values', repmat(estimated_params(3), 9, 1), repmat(fval, 9, 1), repmat(aic, 9, 1), repmat(bic, 9, 1), ...
                'VariableNames', {'SetSize', 'Amplitude', 'Beta', 'Kappa', 'LogLikelihood', 'AIC', 'BIC'});
...
beta2_est = 1 - estimated_params(2); % for Non-redundant Cue conditions
kappa_est = estimated_params(3);
...
for i = 1:length(set_Size)
    ...
    if ~isempty(data_AQS{i}) && isnumeric(data_AQS{i})
        ...
        if any(i == [1, 2, 3]) % Standard conditions
            amplitude = (a2_est * sqrt(2)) / (colour_Num(i)^0.5);
