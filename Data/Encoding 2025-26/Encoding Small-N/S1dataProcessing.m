...
nHigh = sum(dataAll.ResponseTime > 3000);
dataAll = dataAll(dataAll.ResponseTime <= 3000, :);
...
%% ===================== MODEL-READY TABLE EXPORT =====================
% Add after CLEAN & PREP, after cueType/grouping/presDur/RT/precision are defined.

% Absolute circular error: 0..180 degrees
AbsErr = abs(mod(double(precision(:)) + 180, 360) - 180);

% Signed circular error: -180..180 degrees
SignedErr = mod(double(precision(:)) + 180, 360) - 180;

% RT
RT_model = double(RT(:));

% Duration variables
Duration_s = double(presDur(:));
Duration_ms = Duration_s * 1000;

% If durations were already stored in ms, correct automatically
if median(Duration_ms, 'omitnan') > 10000
    Duration_ms = Duration_s;
    Duration_s = Duration_ms / 1000;
end

durLevels = sort(unique(Duration_ms));
durLabels = arrayfun(@(x) sprintf('%dms', round(x)), durLevels, 'UniformOutput', false);
Duration = categorical(Duration_ms, durLevels, durLabels);
Duration = reordercats(Duration, durLabels);

% Session variables
Session = double(dataAll.Session(:));
Session_z = (Session - mean(Session, 'omitnan')) ./ std(Session, 'omitnan');
...
% Use your already-normalised cueType and grouping variables
CueType_model = cueType(:);
Grouping_model = grouping(:);

% Keep rows valid for modelling
validRows = isfinite(AbsErr) & isfinite(SignedErr) & isfinite(RT_model) & ...
            ~isundefined(ID) & ~isundefined(CueType_model) & ...
            ~isundefined(Duration) & ~isundefined(SessionPhase);

modelT = table( ...
    ID(validRows), ...
    Session(validRows), ...
    Session_z(validRows), ...
    SessionPhase(validRows), ...
    CueType_model(validRows), ...
    Duration(validRows), ...
    Duration_ms(validRows), ...
    Grouping_model(validRows), ...
    AbsErr(validRows), ...
    SignedErr(validRows), ...
    RT_model(validRows), ...
    'VariableNames', {'ID','Session','Session_z','SessionPhase', ...
                      'CueType','Duration','Duration_ms','Grouping', ...
                      'AbsErr','SignedErr','RT'} );

% Ensure preferred category order
modelT.CueType = reordercats(categorical(modelT.CueType), {'NR','R'});
modelT.Grouping = reordercats(categorical(modelT.Grouping), {'Grouped','Separated'});

modelT.logRT = log(modelT.RT);

summaryT = groupcounts(modelT, {'ID','Session'});
disp(summaryT);

writetable(modelT, 'S1_smallN_model_ready.csv');
fprintf('\nSaved: S1_smallN_model_ready.csv\n');

%% ===================== MATLAB LME SANITY CHECKS =====================

% Precision model: absolute error
m_abs_main = fitlme(modelT, ...
    'AbsErr ~ CueType * Duration + (1|ID)', ...
    'FitMethod','REML');
disp(m_abs_main);
disp(anova(m_abs_main));

% Precision model with session learning effect
m_abs_session = fitlme(modelT, ...
    'AbsErr ~ CueType * Duration * Session_z + (1|ID)', ...
    'FitMethod','REML');
disp(m_abs_session);
disp(anova(m_abs_session));

% Early vs late sessions only
earlyLateT = modelT(modelT.SessionPhase == "Early" | modelT.SessionPhase == "Late", :);

m_abs_phase = fitlme(earlyLateT, ...
    'AbsErr ~ CueType * Duration * SessionPhase + (1|ID)', ...
    'FitMethod','REML');
disp(m_abs_phase);
disp(anova(m_abs_phase));

% Optional: include Grouping as exploratory predictor
m_abs_grouping = fitlme(modelT, ...
    'AbsErr ~ CueType * Duration * Grouping + (1|ID)', ...
    'FitMethod','REML');
disp(m_abs_grouping);
disp(anova(m_abs_grouping));

% Bias check: signed error
m_bias = fitlme(modelT, ...
    'SignedErr ~ CueType * Duration + (1|ID)', ...
    'FitMethod','REML');
disp(m_bias);
disp(anova(m_bias));
...
% RT model
m_rt = fitlme(modelT, ...
    'logRT ~ CueType * Duration + (1|ID)', ...
    'FitMethod','REML');
disp(m_rt);
disp(anova(m_rt));

% RT model with session
m_rt_session = fitlme(modelT, ...
    'logRT ~ CueType * Duration * Session_z + (1|ID)', ...
    'FitMethod','REML');
disp(m_rt_session);
disp(anova(m_rt_session));
