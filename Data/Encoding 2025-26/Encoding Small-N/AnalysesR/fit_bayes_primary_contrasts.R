...
summarise_effect <- function(data, effect_name) {
  ...
  if (length(vals) == 0) stop("No finite draws for: ", effect_name)
  tibble(
    effect = effect_name,
    median = median(vals),
    mean = mean(vals),
    lower = as.numeric(quantile(vals, 0.025)),
    upper = as.numeric(quantile(vals, 0.975)),
    p_lt_0 = mean(vals < 0),
    p_gt_0 = mean(vals > 0),
    p_in_rope = mean(abs(vals) < rope_abs)
  )
}
...
participant_primary_contrasts <- function(model, participant) {
  message("Contrasts for ", participant)
  ...
  sz_vals <- sort(unique(duration_by_cue_session$Session_z))
  if (length(sz_vals) != 3) stop("Expected 3 Session_z slices")
  duration_by_cue_session <- duration_by_cue_session %>%
    ungroup() %>%
    mutate(SessionSlice = c("early", "average", "late")[match(Session_z, sz_vals)])
...
models <- lapply(participants, function(pid) {
  readRDS(file.path("bayes_primary", paste0("m_", pid, "_abserr.rds")))
})
names(models) <- participants
...
primary_summary <- bind_rows(
  map_dfr(contrasts, "gain_50"),
  map_dfr(contrasts, "duration_summary"),
  map_dfr(contrasts, "catch_avg"),
  map_dfr(contrasts, "catch_by_session"),
  map_dfr(contrasts, "catch_progression"),
  map_dfr(contrasts, "practice_by_cue")
) %>%
...
theory_focus <- primary_summary %>%
  ...
  mutate(
    question = case_when(
      ...
      grepl("duration_effect_late_minus_early", effect) ~ "4. practice change",
      TRUE ~ "other"
    )
  ) %>%
...
cat("\n===== THEORY FOCUS =====\n")
print(as.data.frame(theory_focus), digits = 3)
cat("\nDONE\n")
