...
% 5. Model 1: free kappa, shared psi, grouped vnorm, fitted per participant
%
% Q = [vnormBase, vnormNR, vnormR, eta, psi, a, ter, ...
%      kappa_S2C2NR, ..., kappa_S6C6NR]
%
% vnormBase: S2C2NR, S4C4NR, S6C6NR
% vnormNR:   S4C2NR, S6C2NR, S6C4NR
% vnormR:    S4C2R,  S6C2R,  S6C4R

modelKappaCond.name = "FreeKappa_GroupedVnorm";
modelKappaCond.paramNames = ["vnormBase", "vnormNR", "vnormR", ...
                             "eta", "psi", "a", "ter", ...
                             "kappa_" + condLevels];

lb = [0.01, 0.01, 0.01, 0.01, -1.00, 0.10, 0.00, ...
      repmat(0.01, 1, nCond)];
ub = [20.0, 20.0, 20.0, 8.00,  1.00, 8.00, 1.00, ...
      repmat(30.0, 1, nCond)];
...
for p = 1:numel(fitIDs)
    ...
    evalTimer = tic;
    testNLL = jpcdm_nll_free_kappa_prepared(Qstarts(1, :), dataByCond, condLevels, stFixed, tmax);
    ...
    if useParallel
        ...
        parfor s = 1:nStarts
            ...
            obj = @(q) jpcdm_nll_free_kappa_prepared(q_to_p(q, lb, ub), dataByCond, condLevels, stFixed, tmax);

            qfit = fminsearch(obj, qstart, options);

            allQfit(s, :) = q_to_p(qfit, lb, ub);
            allNLL(s) = jpcdm_nll_free_kappa_prepared(allQfit(s, :), dataByCond, condLevels, stFixed, tmax);
        end
    else
        for s = 1:nStarts
            qstart = p_to_q(Qstarts(s, :), lb, ub);
    ...
    fprintf('\nFinished %s for %s in %.1f minutes.\n', ...
        modelKappaCond.name, uid, elapsedSeconds / 60);
    fprintf('Best start index: %d\n', bestIdx);
    fprintf('Best NLL: %.4f\n', bestNLL);

    fitSummary = table(repmat(uid, numel(modelKappaCond.paramNames), 1), ...
        modelKappaCond.paramNames(:), Qfit(:), lb(:), ub(:), ...
    ...
    condFit = build_free_kappa_cond_fit(Qfit, condLevels, stFixed);
    ...
    fitResultKappaCond(p).uid = uid;
    fitResultKappaCond(p).Qfit = Qfit;
    fitResultKappaCond(p).bestNLL = bestNLL;
    fitResultKappaCond(p).bestIdx = bestIdx;
    fitResultKappaCond(p).allQfit = allQfit;
    fitResultKappaCond(p).allNLL = allNLL;
    fitResultKappaCond(p).paramNames = modelKappaCond.paramNames;
    fitResultKappaCond(p).fitSummary = fitSummary;
    fitResultKappaCond(p).condFit = condFit;
    fitResultKappaCond(p).stFixed = stFixed;
...
function condFit = build_free_kappa_cond_fit(Qfit, condLevels, stFixed)
    nCond = numel(condLevels);
    ...
    condFit = table(condLevels(:), vnormGroup, vnormByCond, kappaFit(:), ...
        repmat(etaFit, nCond, 1), repmat(psiFit, nCond, 1), ...
