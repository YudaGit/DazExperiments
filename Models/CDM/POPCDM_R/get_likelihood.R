...
# This is an adopted demonstration of how to fit the circular model, 
# by getting the summed log likelihood of a set of data by interpolating each 
# trial in the 2D likelihood matrix which can then be optimised.
###############################################################################
source('cdm300.R')
...
Pfull <- c(5, 0,  1,    0,    1,     3,  .15,  0)  # Example parameter vector
tmax  <- 5.1
...
get_likelihood <- function(P, data){
  ...
  # Call cdm300 with the parameter vector P
  CDM <- cdm300(Pcore, tmax, ter, st)
