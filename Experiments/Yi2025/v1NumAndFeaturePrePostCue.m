...
global Visual
Visual = initiate();
trials = TrialMatrix(360, sessionN, participantID, age, timestamp);
practrials = TrialMatrix(13, sessionN, participantID, age, timestamp);
...
try
    ...
    for ii = 1:size(practrials,1)
        ...
        TargetCue(practrials(ii,:));
        WaitSecs(Visual.Durations.PostCueDuration);
    ...
    for ii = 1:size(trials,1)
        ...
        PreCue(trials(ii,:));
        %Cue(trials(ii,:), trials(ii,:).CueOrder_i == 0, trials(ii,:).CueOrder_i == 0)
        WaitSecs(Visual.Durations.PreCueDuration);
        ...
        %Cue(trials(ii,:), trials(ii,:).CueOrder_i == 1)
        TargetCue(trials(ii,:));
        WaitSecs(Visual.Durations.PostCueDuration);
...
function [] = adjustStim(VA5deg)
    ...
    Visual.mouseinit.toofast = 150;
    Visual.mouseinit.tooslow = 1200;
