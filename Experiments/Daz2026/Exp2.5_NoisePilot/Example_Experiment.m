...
% Edited: 06-05-2025
% Version 3: 
% - Enhanced response wheels - done
% - Identicial probe outlines - done
% - C-shape Stimuli (not triangles) - done
% - OKlab Color Space - done
% - No 'Too Slow To Leave Center' check - done
% - 400ms stim and 400ms retention interval, 1200ms pre-cue period.
% - Non-informative precue = Color wheel + Orientation markers - No! 
%    turns out this leaves color wheel after images when it swaps to a
%    white orientation wheel, so we need an alternative 'neutral' wheel.
%    For this I've use a white noise filled wheel. - done
% - Added neutral noise wheel after the trial ends to clear the retina of
%   color so there are no color after images -- I found this to be an issue
%   when swapping from a series of color trials to the white orientation
%   wheel where colors were inverted on my retina. - done
% Kill Switch: 'left-ctrl' + 'L' to quit experiment.
...
global Visual
Visual = initiate();
Visual.PrintScreens = false; % true for stim saving only.
...
try
    ...
    for ii = 1:size(practrials,1)
        ...
        printScreen([s,'Response'], Visual.window);
        ...
        printScreen([s,'Feedback'], Visual.window);
        ...
        fixation(0);
        Screen('Flip', Visual.window);
        WaitSecs(Visual.Durations.RetinalColorReset);

        DrawIntertrialFeedback(practrials,ii);
        printScreen([s,'ITI'], Visual.window);
...
function [] = instructions(n)
    ...
    if n == 1
        inst = ['Welcome to the Color-Orientation task.\n\n\n\n' ...
            'In this task, you will view 1, 2 or 4 colored stimuli.\n\n\n\n' ...
            'Using your mouse, you will be cued to report either\n\n' ... 
            'the line orientation or color of a probed stimulus.'];
    elseif n == 2
        inst = ['This is the PreCue practice block. Before the stimulus, you will\n\n' ...
            'be cued to report either color (color wheel) or orientation (spiky wheel).\n\n'];
    elseif n == 3
