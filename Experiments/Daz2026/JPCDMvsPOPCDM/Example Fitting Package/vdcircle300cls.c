...
void vdcircle300cls(double *T, double *Gt, double *Theta, double *Ptheta, double *Mt, 
    ...
    double w, two_pi, h, v1, v2, eta1, eta2, sigma, a, sigma2, eta1onsigma2, eta2onsigma2, mt, 
           G11, G12, G21, G22, Girs1, Girs2, tscale, t1, t2, t3, t4, 
           mtscale, totalmass;
    double munorm;
    int i, k;
   ...
   for (k = 0; k < sz; k++) {
       ...
       /* mexPrintf("k= %6d tscale = %10.6f \n", k, tscale);    */
        ...
        for (i = 0; i < nw; i++) {
            ...
            /*t1 = (G12 * G12);
            t2 = G11;
            t3 = (v1 * v1) / (eta1 * eta1) / 2;
            t4 = exp((G12 * G12) / G11 - (v1 * v1) / (eta1 * eta1) / 2); */
            Gt[(nw + 1) * k + i] = tscale * Girs1 * Girs2 * Gt0[k] / two_pi; 
            /*if (i==29) { 
              mexPrintf("k= %6d t1=%9.5f,t2=%9.5f, t3=%9.5f, t4=%9.5f\n", t1, t2, t3, t4);   
            } */ 
            /* t3 goes large with small eta, t1 goes large with large eta - exponent of square is problem */           
        }
        /* Close the domain */
