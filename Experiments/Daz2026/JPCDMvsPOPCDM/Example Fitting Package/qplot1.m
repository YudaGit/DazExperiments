function qplot1(fitfunc, Pvar, Pfix, Sel, Data_in, ybounds);
% ----------------------------------------------------------------------
% Generic Q x Q plot for Experiment 1
% Empirical and RT fitted RT quantiles as a function of theta.
% Equal-mass theta bounds, RTs filtered on [minrt, maxrt]
%       qplot1(@fitfunc, Pvar, Pfix, Sel, Data_in, {[ymin,ymax]});
% ----------------------------------------------------------------------
   if nargin < 6
      ybounds = [0.2, 2.0];
   end   
    ...
    errmg2 = 'QPLOT1: Wrong size data matrix, exiting...';
    ...
    qploti(co, axhandle(1), Data{1}, gtma, ta, thetaa, tmax, minrt, maxrt, edlab{1}, 0, ybounds);
    qploti(co, axhandle(2), Data{2}, gtmb, tb, thetab, tmax, minrt, maxrt, edlab{2}, 0, ybounds);
    qploti(co, axhandle(3), Data{3}, gtmc, tc, thetac, tmax, minrt, maxrt, edlab{3}, 0, ybounds);
    qploti(co, axhandle(4), Data{4}, gtmd, td, thetad, tmax, minrt, maxrt, edlab{4}, 1, ybounds);
...
% =======================================================================
  ...
  % Empirical RT quantiles in accuracy bins
   ...
   set(gca, 'YLim', [ybounds(1), ybounds(2)]); 
   if do_xlabel
