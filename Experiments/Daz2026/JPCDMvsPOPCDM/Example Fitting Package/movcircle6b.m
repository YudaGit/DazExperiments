function [ll,qaic,qbic, Pred,Gstuff] = movcircle6b(Pvar, Pfix, Sel, Data, trace)
% ==========================================================================
% Circular diffusion model for Elaine's continuous movement task (2p, 2D)
% (Like previous fits but simplified (4 x cat, 1 x Ter, 1 x st)
% Modified <movspshere1d> to reduce np. (previously fitted with {m1n17})
% 4 instead of 6 categories and 1 x Ter, 1 x st
% May 25, 2021 
% l. 216 modified to take 4-column data structure (from Dataj) from new
% <makesub> P1aj...P8aj - May 25 2021
% 4 x eta, sets eta1b = eta2b = 0
% Sets the tangential components for zero, allows elevation to vary.
% Four categories, 1 category strength
% 1a: 2 x Ter, 2 x st
% Two coherence conditions. 
%   [ll,qaic,qbic,Pred,Gstuff] =  movcircle6b(Pvar, Pfix, Sel, Data, trace)
...
%% ===========================================================================
   ...
   % --------------------------------------------------------------------------------------- 
    ...
    if any(P - Ub > 0) | any(Lb - P > 0)
     ...
     parfor i = 1 : ncond
          [Gstuffi, Predi, lli] = vacircle12j(P(i,:), Data{i}, Abias, Bbias);
          Gstuff1{i} = Gstuffi{1};
          Gstuff2{i} = Gstuffi{2};
          Gstuff3{i} = Gstuffi{3};
          Pred1{i} = Predi{1};
...
function [Gstuff, Pred, sll0] = vacircle12j(Pj, Dataj, Abias, Bbias)
% ===============================================================================================
...
% ===============================================================================================
   ...
   [t, gt, thetas, theta, ptheta, mtheta] = vjoint5density(Pj(1:9), Abias, Bbias, tmax, nw1, nv1, nt);
   % Filter zeros  
...
function [t, gt, thetas, thetaerr, ptheta, mtheta] = vjoint5density(P, Abias, Bbias, tmax, nw1, nv1, nt)
% ===============================================================================================
