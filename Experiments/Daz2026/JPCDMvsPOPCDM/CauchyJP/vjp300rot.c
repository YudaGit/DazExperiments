/*% ==========================================================================
%  Circular diffusion model 300-step C version. Wrapped-Cauchy front layer.
...
%   P = [nunorm, kappa, eta_rad, eta_tan, phi, sigma, a]
%  Jones-Pewsey psi is compiled at -1, giving the wrapped Cauchy.
%  
...
#define NP 7     /* psi=-1 is compiled; eta_rad and eta_tan are runtime inputs */
...
int bessel_roots_initialized = 0;
...
void dserafin(double *T, double *Gt0, double *P0, double tmax) {
   ...
   */
    ...
    jv1 = J0k[0]; /* initialized by dhamana before dserafin is called */
...
double wrappedcauchy(double theta, double varphi, double kappa) {
   /* Jones-Pewsey psi=-1. This form integrates to one on the circle. */
   double denominator;
   denominator = cosh(kappa) - sinh(kappa) * cos(theta - varphi);
   return 1.0 / (2.0 * pi * denominator);
}
...
void vjp300rot(double *T, double *Gt, double *Theta, double *Ptheta, double *Mt, 
    ...
    double v1, v2, phi, nunorm, kappa, sigma, a, eta1, eta2, 
    ...
    for (j = 0; j < njsteps; j++) {
       ...
       ProbMu[j] = wrappedcauchy(ThetaMu[j], 0, kappa); 
       sumprob += ProbMu[j];
