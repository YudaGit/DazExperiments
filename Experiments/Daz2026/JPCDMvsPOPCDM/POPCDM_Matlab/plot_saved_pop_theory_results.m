function figureHandles = plot_saved_pop_theory_results(resultFile, targetUIDs)
%PLOT_SAVED_POP_THEORY_RESULTS Recreate H0a/H0b/H1 diagnostics without refitting.
%
%   plot_saved_pop_theory_results
%   plot_saved_pop_theory_results("TheoryFits/pop_theory_H0a_full_results.mat")
%   plot_saved_pop_theory_results("TheoryFits/pop_theory_H1_full_results.mat", "YL")
%   plot_saved_pop_theory_results(..., ["AQ","ES"])
    ...
    if strlength(resultFile) == 0
        defaultDir = fullfile(thisDir, 'TheoryFits');
        [fileName, pathName] = uigetfile( ...
            '*.mat', 'Select saved theory results', defaultDir);
        if isequal(fileName, 0)
            figureHandles = gobjects(0);
            return
        end
        resultFile = string(fullfile(pathName, fileName));
    elseif ~contains(resultFile, filesep) && ~startsWith(resultFile, "/")
        cand = fullfile(thisDir, 'TheoryFits', resultFile);
        if isfile(cand)
            resultFile = string(cand);
        end
    end
    ...
    if ~isfield(saved, 'fitResults') || isempty(saved.fitResults)
        error('Result file has no fitResults.');
    end
    ...
    if isfield(saved, 'hyp') && isfield(saved.hyp, 'name')
        hypName = string(saved.hyp.name);
    else
        hypName = string(fitResults(1).hypothesis);
    end

    tmax = local_get(saved, 'tmax', 3.0);
    nw = local_get(saved, 'nw', 50);
    h = local_get(saved, 'h', tmax / 300);

    uids = string({fitResults.uid});
    if ~isempty(targetUIDs)
        keep = ismember(uids, targetUIDs);
        if ~any(keep)
            error('None of the requested IDs were in the result file.');
        end
    ...
    for p = 1:numel(fitResults)
        ...
        dp = d(d.uid == uid, :);
        [angleFigure, rtFigure] = local_plot_one( ...
            uid, dp, fitResults(p).condFit, condLevels, nw, h, tmax, hypName);
...
function value = local_get(saved, fieldName, defaultValue)
...
function d = local_prepare_data(dataFile, targetIDs, condLevels)
    ...
    if any(isnan(nItems)) || any(isnan(nColors))
        error('Unexpected num_items or ColorN while preparing plot data.');
    end
    ...
    if any(~known)
        error('Unexpected condition while preparing plot data.');
    end
    d.rAngle = d.response_error * pi / 180;
    d.rt = d.response_RT / 1000;
end

function [angleFigure, rtFigure] = local_plot_one( ...
    ...
    angleFigure = figure('Name', char(uid + " POPCDM " + hypName + " angle"));
    tiledlayout(3, 3);
    ...
    rtFigure = figure('Name', char(uid + " POPCDM " + hypName + " RT"));
    tiledlayout(3, 3);
