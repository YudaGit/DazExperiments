...
%   design.ConditionSpec  : table of valid ItemN, RedundantN and CueType
%                           combinations.  This avoids invalid Cartesian
%                           combinations when set-size varies.
%   design.presDurList    : vector of presentation times (s)
%   design.PracticePresDurList : vector of practice presentation times (s)
...
% Build blocks from valid condition rows.  CueType remains 'NR' for
% all-unique baseline arrays (RedundantN = 0), preserving the Exp. 1 schema.
mainCoreTbl = makeCore(design.ConditionSpec, design.presDurList, design.retDurList);
pracCoreTbl = makeCore(design.ConditionSpec, design.PracticePresDurList, design.retDurList);

% Inflate rows for practice & main blocks
pracRows = repelem(pracCoreTbl, design.PracticeReps, 1);
mainRows = repelem(mainCoreTbl, design.MainReps, 1);
...
%───────────────────Build valid condition rows────────────────────────
function coreTbl = makeCore(conditionSpec, presDurList, retDurList)
% CONDITIONSPEC contains only legal ItemN/RedundantN/CueType combinations.
% Cross duration and retention factors with these rows while leaving the
% trial-data schema unchanged.

nConditions = height(conditionSpec);
core = allcomb(1:nConditions, presDurList, retDurList);

conditionIdx = cell2mat(core(:,1));
coreTbl = conditionSpec(conditionIdx, {'ItemN','RedundantN','CueType'});
coreTbl.PresDur = cell2mat(core(:,2));
coreTbl.RetDur  = cell2mat(core(:,3));
end
