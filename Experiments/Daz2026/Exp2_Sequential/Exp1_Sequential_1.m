%======================================================================
% Exp1_Sequential_1.m   (Continuous colour-report)
% Daz Liu 2025 PhD Experiment 2, Redundancy in Sequence
%
% 3 sequence conditions (Nseq=2,4,6), single presDur, adjustable segment & ISI
%======================================================================

clear; close all; clc;
sca; Screen('CloseALL');
format shortg;
InitializeMatlabOpenGL;   % ensure the OpenGL runtime is fully up
AssertOpenGL;              
...
% design.CueType      = {'R' 'NR'};
% design.presDurList  = [0.2];
% design.retDurList   = 0.75;
% design.PracticeReps = 1;
% design.MainReps     = 30;

design.ItemN        = 6;                    % total items per display
design.RedundantN   = 3;                    % number of redundant items
design.CueType      = {'R','NR'};           % cue target type
design.SeqType      = [1 2 3];              % 1→2 segments, 2→4 segments, 3→6 segments
design.NseqList     = [2 4 6];              % map SeqType to number of segments
design.presDurList  = 0.20;                 % fixed segment display (s)
design.retDurList   = 0.75;                 % final retention (s)
design.SegmentDur   = 0.20;                 % segment display duration (s)
design.ISI          = 0.20;                 % inter‐segment interval (s)
design.PracticeReps = 0;                    % no practice
design.MainReps     = 3;                   % main block repetitions

% Generate trials
[pracTrials, expTrials] = TrialMatrixSeq(design, ...
                         sessionN, participantID, age, timestamp);

% Check table output
disp(tabulate(pracTrials.RedundantN)); 
disp(tabulate(expTrials.RedundantN));
% Establish textures and calibration
VA5deg         = calibrateMonitor();        % helper: returns pixels for 5° VA
adjustStim(VA5deg);                         % helper: updates V.stim sizes
wheelTexture    = DrawWheel();              % helper: offscreen color wheel
orientTexture   = DrawOrientationWheel();   % helper: orientation wheel
neutralTexture  = DrawNeutralWheel();       % helper: neutral wheel
...
try
    ...
    CueCreation(false); % flase = no display
    ...
    for ii = 1:height(expTrials)
        ...
        % 2) Build sequence segments
        cols = trial.Colors{1};              % 1×6 colour indices
        locs = trial.StimulusLocations{1};   % 1×6 angles
        switch trial.SeqType
            case 1  % two segments: grouped vs unique
                u = unique(cols); cnt = histc(cols,u);
                redC = u(cnt>1);
                idxR = find(cols==redC);
                idxN = find(cols~=redC);
                segs = {idxR, idxN};
            case 2  % four segments: three singletons + group
                u = unique(cols); cnt = histc(cols,u);
                redC = u(cnt>1);
                idxR = find(cols==redC);
                idxN = find(cols~=redC);
                if rand<.5
                    segs = {idxN(1), idxN(2), idxN(3), idxR};
                else
                    segs = {idxR, idxN(1), idxN(2), idxN(3)};
                end
            case 3  % six singletons, clockwise starting at StartLocation
                start = trial.StartLocation;
                order = mod((start-1)+(0:5),6)+1;
                segs  = arrayfun(@(k){order(k)},1:6);
        end

        % 3) Present each segment
        for s = 1:trial.Nseq
            DrawStimulusSegment(trial, segs{s}, wheelTexture, orientTexture);
            Screen('Flip', win);
        ...
        [expTrials.MouseX{ii}, expTrials.MouseY{ii}, ...
         ...
         expTrials.Precision(ii)] = ...
            GetResponse(trial, wheelTexture, orientTexture);
        ...
        % 7) Feedback
        DrawWheelFeedback(expTrials(ii,:), wheelTexture, orientTexture);
        ...
        % 8) Intertrial feedback
        DrawIntertrialFeedbackFast(...
            expTrials(1:ii,:), win, V.windowRect, height(expTrials));
    end
...
% Display Functions
%========================================================
function DrawStimulusSegment(trial, idxList, wheelTex, orientTex)
    global V
    fixation(0);
    locs = trial.StimulusLocations{1};   % 1×6 numeric
    cols = trial.Colors{1};              % 1×6 numeric
    cMap = V.color.map(1 : size(V.color.map,1)/360 : end, :);

    for k = 1:numel(idxList)
        j = idxList(k);   % ← parentheses, returns the k-th index
        ...
        rect = CenterRectOnPointd([-V.stim.radius -V.stim.radius V.stim.radius V.stim.radius], x, y);
