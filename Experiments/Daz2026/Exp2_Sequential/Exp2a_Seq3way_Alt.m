%======================================================================
% Exp2a_SeqSingle_Final.m   (Continuous colour-report) Version.29072025
...
design.ItemNList    = [6];
design.RedundantMap = {[0 3] };
design.CueBalance   = {'noRedundant','cueR','cueNR'}; 
design.PracticeReps = 0;    % reps per block in practice
design.MainReps     = 20;   % reps per block in main
design.presDur      = 0.20;
design.retDur       = 0.75;
design.SegmentDur   = 0.20;
design.ISI          = 0.20;

% Generate practice & main tables
[pracTrials, expTrials] = TrialMatrixSeq3way_Alt(design, sessionN, participantID, age, timestamp);

validateSegments(pracTrials);
validateSegments(expTrials);
...
try
    ...
    % — Practice Loop —
    for ii = 1:height(pracTrials)
        tr = pracTrials(ii,:);
        % before the loop:
        Screen('FillRect', win, V.patch.bg);    % clear to gray
        fixation(0);                            % draws cross (no flip)
        drawRings(tr);                          % draws all 1px rings (no flip)
        Screen('Flip', win);                    % show fixation+rings
        WaitSecs(V.Durations.FixationDuration); 
        
        % 1) Sequence of singletons
        allSegs = tr.SegmentOrder{1};
        for s = 1:numel(allSegs)
            idxList = allSegs{s};
            if iscell(idxList), idxList = idxList{1}; end
            idxList = double(idxList(:))';  % normalized
        
            if isempty(idxList) || any(idxList<1 | idxList>tr.ItemN)
                warning('Bad Segment at trial %d, step %d: %s', ii, s, mat2str(idxList));
                continue; % or error out if you prefer
            end
        
            Screen('FillRect', win, V.patch.bg);
            fixation(0); drawRings(tr);
            DrawStimulusSegment(tr, idxList);
            Screen('Flip', win);  WaitSecs(design.SegmentDur);
        
            if s < numel(allSegs)
                Screen('FillRect', win, V.patch.bg);
                fixation(0); drawRings(tr);
                Screen('Flip', win);  WaitSecs(design.ISI);
            end
        end
        
        % 2) Final retention (just fixation + rings)
        Screen('FillRect', win, V.patch.bg);
        fixation(0);
        drawRings(tr);
        Screen('Flip', win);
        ...
        % 5) Response
        [ pracTrials.MouseX{ii}, pracTrials.MouseY{ii}, ...
          pracTrials.MouseAngles{ii}, pracTrials.MouseDistances{ii}, ...
          pracTrials.MouseTime{ii}, pracTrials.ResponseTime(ii), ...
          pracTrials.ResponseAngle(ii), pracTrials.DerotatedResponseAngle(ii), ...
          pracTrials.Precision(ii) ] = ...
            GetResponse(tr, wheelTex);

        % 6) Speed check & feedback
        [ pracTrials.MouseInitTooSlow(ii), pracTrials.MouseInitTooFast(ii), ...
          pracTrials.TrialTooSlow(ii) ] = speedCheck(pracTrials(ii,:));
        DrawWheelFeedback(pracTrials(ii,:), wheelTex);
        penalty  = pracTrials.TrialTooSlow(ii)*V.Durations.FeedbackPenaltyDuration;
        standard = ~pracTrials.TrialTooSlow(ii)*V.Durations.FeedbackDuration;
        WaitSecs(penalty+standard);

        % 7) Inter‐trial feedback
        DrawIntertrialFeedbackFast( pracTrials(1:ii,:), win, V.windowRect, height(pracTrials) );
    end
...
function drawRings(tr)
    % Draw one 1px white ring at each stimulus locus, using the pedestal radius
    global V
    locs = tr.StimulusLocations{1};
    for j = 1:numel(locs)
        theta = deg2rad(locs(j));
        x = V.centerX + V.stim.positionradius * cos(theta);
        y = V.centerY - V.stim.positionradius * sin(theta);
        % ring exactly at pedestal radius
...
function DrawStimulusSegment(trial, idx)
    ...
    cols = trial.Colors{1};
    cMap = V.color.map(1:end/360:end,:);

    % simple bounds guard (prevents "indices must be positive integers")
    assert(all(idx>=1 & idx<=trial.ItemN), 'Bad index in segment.');

    theta = deg2rad(locs(idx));
    xs    = V.centerX + V.stim.positionradius * cos(theta);
    ys    = V.centerY - V.stim.positionradius * sin(theta);
    rgbs  = cMap(cols(idx),:);

    sq = [-V.stim.radius -V.stim.radius  V.stim.radius  V.stim.radius];
    for ii = 1:numel(idx)
        rect = CenterRectOnPointd(sq, xs(ii), ys(ii));
        Screen('FillOval', V.window, rgbs(ii,:), rect);
    end
end
...
% =======================================================
function validateSegments(T)
for i = 1:height(T)
    N = T.ItemN(i); segs = T.SegmentOrder{i};
    flat = [segs{:}];
    assert(numel(flat) == N, 'Trial %d: drew %d stimuli (should be %d).', i, numel(flat), N);
    assert(isequal(sort(unique(flat)), 1:N), 'Trial %d: bad indices: %s', i, mat2str(flat));
end
disp('SegmentOrder OK.');
end
...
% Saving data
    global V
    saveDir = 'Daz25Seq3wayData';
    if ~isfolder(saveDir)
        mkdir(saveDir);
        disp(['Save Data Directory Created: ', saveDir]);
    end
    fname = sprintf('Seq3wayData_%s_sess%d_%s.mat', ...
