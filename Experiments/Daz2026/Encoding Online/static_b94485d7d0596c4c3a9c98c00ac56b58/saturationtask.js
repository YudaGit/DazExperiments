    ...
    class ConfidenceWheelPlugin {
        ...
        trial(display_element, trial) {
            ...
            if (window.pxPerMm) {
                ...
                startingradius = Math.round(window.outerHeight * 0.035); // fallback
            }
            // Invisible circle radius: 4.5 degrees VA (~22.5mm at calibrated viewing distance)
            var patchradius;
                if (window.pxPerMm) {
                    var desired_circle_radius_mm = 22.5; // 4.5 degree VA
                    patchradius = Math.round(window.pxPerMm * desired_circle_radius_mm);
                } else {
                    patchradius = window.outerHeight * 0.15; // fallback: ~15% of window height
                    }
            var targetN = Math.floor(Math.random() * (trial.choice_colors.length ));
            ...
            // circle2.arc(midx, midy, patchradius, 0, 2 * Math.PI);
            // ctx.strokeStyle = 'green';
            // ctx.stroke(circle2);
            // display_element.insertBefore(canvas, null); 
            
            var indiv_patch_radius;
            if (window.pxPerMm) {
                // desired radius in millimeters (changeable)
                var desired_patch_radius_mm = 5; // 5 mm radius => 10 mm diameter
                indiv_patch_radius = Math.round(window.pxPerMm * desired_patch_radius_mm);
            } else {
                indiv_patch_radius = Math.round(window.outerHeight * 0.024)
            }
            ...
            if (trial.draw_wheel == true){
                ...
                if (window.pxPerMm) {
                    var desired_wheel_outer_radius_mm = 60; // Outer edge of color wheel
                    var desired_wheel_width_mm = 5; // Width of the wheel ring
                    ...
                    outer_radius = Math.round(window.outerHeight * 0.354);
                    inner_radius = Math.round(outer_radius - window.outerHeight * 0.028);
                }
                ...
                for (var ii = 0; ii < trial.patch_positionalangle.length; ii++){
                    ...
                    patches.arc(midx + patchradius * Math.cos( trial.patch_positionalangle[ii] * Math.PI/180 ), 
                                midy + patchradius * Math.sin( trial.patch_positionalangle[ii] * Math.PI/180 ), 
                                indiv_patch_radius * 1.02, 0, 2 * Math.PI);
                    if (ii == targetN){
                        ctx.lineWidth = 3
                        ctx.strokeStyle = 'white'
                    } else {
            ...
            var points = []
            var confidence_rt = []
            var confidence_arc_rad = []

            var endtrial = false
            var mousecheckinterval
            var target_in_bounds = []
            ...
            if (trial.draw_wheel == true & endtrial == false){
                ...
                function checkMousePosition() {
                    ...
                    if (trial.draw_wheel == true){
                        ...
                        } // If a first response hasn't been made END
                    }

                    // Now include checks on interval timers - allows graphics to not overwork.
                    if (interval_count == interval){
                        // console.log(`Mouse position: (${x_mouse}, ${y_mouse})`, interval_count);
                        interval_count = 0
                        ...
                        if (trial.draw_wheel == true){
                            ...
                            if (responded == true){
                                ...
                                if ((distance >= inner_radius * .5 & distance <= outer_radius * 1.5) | finished == true){
                                    if (finished == false){
                                        var posX = x_mouse
                                        var posY = y_mouse
                                        finX[0] = x_mouse
                                        finY[0] = y_mouse
                                        var colorstroke = 'white'
                                        var strokewidth = 2
                                    } else {
                                    ...
                                    var rad2 = 2 * Math.atan( posY / ( posX + Math.sqrt( posX * posX + posY * posY ) ) )
                                    // var deg2 = rad2 * (180/ Math.PI)
                                    radDiff = ((radians[0] - rad2) + Math.PI) % (2 * Math.PI) - Math.PI
                                    if (radDiff < -Math.PI){radDiff += 2*Math.PI}
                                    // var diff = radDiff * 180/Math.PI
                                    
                                    // Clear Confidence Arc
                                    ctx.beginPath()
                                    ctx.arc(midx, midy, outer_radius * 1.1, 0, 2*Math.PI)
                                    ...
                                    // Draw Confidence Arc
                                    ctx.beginPath();
                                    var pointA = degrees[0] * Math.PI/180 + radDiff
                                    var pointB = degrees[0] * Math.PI/180 - radDiff
                                    ctx.arc(midx, midy, outer_radius * 1.1, pointA, pointB, radDiff > 0 ? true: false)
                                    ...
                                    if (finished == true){
                                        ...
                                        // Draw The Response Location in Updated Color
                                        ctx.strokeStyle = colorstroke
                                        ctx.beginPath();
                                        ctx.moveTo(midx + outer_radius * 1.1 * Math.cos(-radians[0]), midy + outer_radius * 1.1 * Math.sin(-radians[0]) )
                                        ctx.lineTo(midx + inner_radius * Math.cos(-radians[0]), midy + inner_radius * Math.sin(-radians[0]) )
                            ...
                            canvas.addEventListener('mousedown', function(event){
                                if (responded == true & finished == false){
                                    finished = true
                                    endtrial = true
                                    confidence_rt.push(performance.now() - start_time - rt[0])
                                    confidence_arc_rad.push( radDiff )
                                    
                                    if (Math.abs(resp_error[0]) <= Math.abs( radDiff * 180/Math.PI )){
                                        points.push(100 - Math.abs(radDiff * 180/Math.PI) / 180 * 100)
                                        points[0] = Math.round(points[0] * (1 - Math.abs((radDiff * 180/Math.PI)/ 180)))
                                        target_in_bounds.push('true')
                                    } else {
                                        points[0] = 0
                                        target_in_bounds.push('false')
                                    }
                                    ...
                                    ctx.fillText( points[0] + ' Points!', midx, canvas.height * .3)
                                    display_element.insertBefore(canvas, null);
                                    end_trial()
                                }
            ...
            var trial_data = {
                ...
                confidence_arc_rad: confidence_arc_rad,
