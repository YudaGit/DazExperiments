...
P.kappaHighNoise =  5;       % wide VM (low evidence)
    ...
    switch stage
        ...
        case 7
            info = sprintf(['Stage 7: Two-Interval Same Location\n' ...
                            'Noise: High (kappa = %.2f)\n' ...
                            'Set Size: 1 (two intervals)'], P.kappaHighNoise);
            action = stage_click_to_repeat(win, @() stage_two_interval_same_loc(V, 90, P.kappaHighNoise, P), info);
        case 8
            info = sprintf(['Stage 8: Two-Interval Different Location\n' ...
                            'Noise: High (kappa = %.2f)\n' ...
                            'Set Size: 1 (two intervals)'], P.kappaHighNoise);
            action = stage_click_to_repeat(win, @() stage_two_interval_diff_loc(V, 90, 270, P.kappaHighNoise, P), info);
        otherwise
            action = 'quit';
    end
...
presentNoisySquareAt(V, targetHueDeg, kappa, angleDeg, P.durMs, [], P.cMap360_255, [], false, false);
end
...
repPattern = makeNoisyPattern(V, baseHue, kappa, [], P.cMap360_255);

FillBG(V); drawFixation(V,[.25 .25 .25],[.75 .75 .75]);
presentNoisySquareAt(V, baseHue,    kappa, angles(1), P.durMs, [], P.cMap360_255, repPattern, true, false); % R replica
presentNoisySquareAt(V, uniqueHue1, kappa, angles(2), P.durMs, [], P.cMap360_255, [],         true, false); % U new
presentNoisySquareAt(V, baseHue,    kappa, angles(3), P.durMs, [], P.cMap360_255, repPattern, true, false); % L replica
presentNoisySquareAt(V, uniqueHue2, kappa, angles(4), P.durMs, [], P.cMap360_255, [],         true, false); % D new
...
hue = randi([0 359]);
pat = makeNoisyPattern(V, hue, kappa, [], P.cMap360_255);

% interval 1
FillBG(V); drawFixation(V,[.25 .25 .25],[.75 .75 .75]);
presentNoisySquareAt(V, hue, kappa, angle1, P.durMs, [], P.cMap360_255, pat, false, false);
...
FillBG(V); drawFixation(V,[.25 .25 .25],[.75 .75 .75]);
presentNoisySquareAt(V, hue, kappa, angle2, P.durMs, [], P.cMap360_255, pat, false, false);
% Note: presentNoisySquareAt already waits for P.durMs internally, no extra WaitSecs needed
end
...
% interval 1
FillBG(V); drawFixation(V,[.25 .25 .25],[.75 .75 .75]);
presentNoisySquareAt(V, hue, kappa, angleDeg, P.durMs, [], P.cMap360_255, pat, false, false);
% Note: presentNoisySquareAt already waits for P.durMs internally, no extra WaitSecs needed
