%======================================================================
% Exp2_SeqGS_Final.m   (Continuous colour-report) Version.29072025
...
[pracTrials, expTrials] = TrialMatrixSeqGS(design, ...
...
try
    ...
    for ii = 1:height(pracTrials)
        ...
        % 2) segments as per tr.SegmentOrder
        allSegs = tr.SegmentOrder{1};
        for seg = 1:numel(allSegs)
            idxList = allSegs{seg};          % <-- now a 1×k numeric vector
...
function DrawStimulusSegment(trial, idxList)
    ...
    % if someone accidentally wrapped twice, flatten:
    if iscell(idxList), idxList = [idxList{:}]; end  
    assert(isnumeric(idxList), 'Segment indices must be numeric');

    locs = trial.StimulusLocations{1};  % e.g. [90 210 330]
    cols = trial.Colors{1};             % e.g. [10 200 300]
    step = size(V.color.map,1)/360;
    cMap = V.color.map(1:step:end,:);
