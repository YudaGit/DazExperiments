%======================================================================
% Exp2_SeqGG_Final.m   (Continuous colour-report) Version.29072025
...
clear; close all; clc;
sca; Screen('CloseAll');
InitializeMatlabOpenGL; KbName('UnifyKeyNames');
global V
...
% window & settings
V = initiate(); win = V.window;
V.PrintScreens = false;
...
% build trial tables
[pracTrials, expTrials] = TrialMatrixSeqGG(design, ...
...
try
 ...
 % — Practice Loop —
  for ii = 1:height(pracTrials)
    ...
    % 1) Pre‐trial fixation + rings
    Screen('FillRect', win, V.patch.bg); fixation(0); drawRings(tr);
    Screen('Flip', win); WaitSecs(V.Durations.FixationDuration);

    % 2) Two segments
    segs = tr.SegmentOrder{1};        % 1×2 cell array of index vectors
    for s = 1:2
      idx = segs{s};                  % e.g. [2 5] or [3]
      Screen('FillRect', win, V.patch.bg); fixation(0); drawRings(tr);
      DrawStimulusSegment(tr, idx);
    ...
    % 3) Retention
    Screen('FillRect', win, V.patch.bg); fixation(0); drawRings(tr);
    Screen('Flip', win); WaitSecs(design.retDur);

    % 4) Response & feedback → write back into pracTrials
    [ pracTrials.MouseX{ii}, pracTrials.MouseY{ii}, ...
      ...
      pracTrials.Precision(ii) ] = GetResponse(tr, wheelTex);
  ...
  for ii = 1:height(expTrials)
    ...
    % 4) Response & feedback → write back into expTrials
    [ expTrials.MouseX{ii}, expTrials.MouseY{ii}, ...
      ...
      expTrials.Precision(ii) ] = GetResponse(tr, wheelTex);
...
function DrawStimulusSegment(trial, idxList)
  global V
  locs = trial.StimulusLocations{1};
  cols = trial.Colors{1};
  % pick out every 360th row from the color map
  cMap = V.color.map(1:size(V.color.map,1)/360:end,:);
  for k = idxList
    theta = deg2rad(locs(k));
    x     = V.centerX + V.stim.positionradius * cos(theta);
    y     = V.centerY - V.stim.positionradius * sin(theta);
    rgb   = cMap(cols(k), :);
    rect  = CenterRectOnPointd([-V.stim.radius -V.stim.radius ...
                                V.stim.radius  V.stim.radius], x, y);
...
% Saving data
    global V
    saveDir = 'Daz25SeqSingleData';
    if ~isfolder(saveDir)
        mkdir(saveDir);
        disp(['Save Data Directory Created: ', saveDir]);
    end
    fname = sprintf('SeqSingleData_%s_sess%d_%s.mat', ...
