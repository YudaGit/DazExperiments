...
design.MainReps     = 50;   % reps per block in main
design.presDur      = 0.15;
design.retDur       = 0.75;
...
[pracTrials, expTrials] = TrialMatrixSeq3way(design, sessionN, participantID, age, timestamp);
...
try
    ...
    for ii = 1:height(pracTrials)
        ...
        allSegs = tr.SegmentOrder{1};                 % cell array of step indices
...
function DrawStimulusSegment(trial, idx)
    ...
    % robust 360-row lookup
    cMap = V.color.map(round(linspace(1,size(V.color.map,1),360)), :);
