%======================================================================
% Exp2a_SeqSnG_Final.m   (Continuous colour-report) Version.11082025
% Daz Liu 2025 PhD Experiment 2a, Redundancy in Sequence, singleton vs group
%
% Set size 4 and 6
% Baseline condition: only unique colors
% Singleton redundancy condition: always force redundant items to be adjascent
%======================================================================
...
[pracTrials, expTrials] = TrialMatrixSeqSingle(design, ...
...
try
    ...
    for ii = 1:height(pracTrials)
        ...
        for k = 1:tr.ItemN
            % draw stimulus k on top of background+fixation+rings
            Screen('FillRect', win, V.patch.bg);
            fixation(0);
            drawRings(tr);
            DrawStimulusSegment(tr, k);         % now draws that one spot
            Screen('Flip', win);                % show it
            WaitSecs(design.SegmentDur);        % hold it
        
            if k < tr.ItemN
                % ISI: go back to just fixation + rings
                Screen('FillRect', win, V.patch.bg);
                fixation(0);
                drawRings(tr);
                Screen('Flip', win);
