% Check trial counts per condition for HomoInte data across all sessions
% This script loads all session files and counts trials by condition

fprintf('=== HomoInte Data Analysis: Trial Counts by Condition ===\n\n');

% Data directory
dataDir = 'Noise Pilot HomoInte Data';
...
for i = 1:length(files)
    % Extract session number from filename: HomoInte_*_sessN_*.mat
...
if istable(allTrials)
    ...
    % Count by Condition × Noise Level
    if ismember('Condition', allTrials.Properties.VariableNames) && ...
    ...
    % Count by Condition × Set Size × Noise Level
    if ismember('Condition', allTrials.Properties.VariableNames) && ...
       ismember('ItemN', allTrials.Properties.VariableNames) && ...
       ismember('NoiseLevel', allTrials.Properties.VariableNames)
        fprintf('=== Trial Counts by Condition × Set Size × Noise Level ===\n\n');
        conditions = unique(allTrials.Condition);
        itemNs = unique(allTrials.ItemN);
        noiseLevels = unique(allTrials.NoiseLevel);
