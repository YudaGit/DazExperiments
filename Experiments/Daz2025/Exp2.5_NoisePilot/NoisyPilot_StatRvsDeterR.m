...
design.BaselineReps = 5;              % N trials per N×Noise combination (Setsize × Noise × BaselineReps)
design.HomoReps     = 20;              % N trials per N×Noise combination (Setsize × Noise × HomoReps)
...
if DebugVerify && DebugNoPTB
    ...
    [pracTrials, expTrials] = TrialMatrixSeq3way_StatRvsDeterR(design, sessionN, participantID, age, timestamp);
...
P.samplingMode   = 'statistical'; % 'statistical' or 'deterministic'
...
% Saving data for Pilot: Statistical vs Deterministic Redundancy
    global V
    saveDir = 'Noise Pilot StatRvsDeterR Data';
    if ~isfolder(saveDir)
        mkdir(saveDir);
        disp(['Save Data Directory Created: ', saveDir]);
    end
    fname = sprintf('HomoInte_%s_sess%d_%s.mat', ...
...
% - Stimulus CENTERS lie on the 5° circle (radius = VA5deg/2).
    ...
    V.square.B             = 8;       % B×B tiles (10×10 grid)
